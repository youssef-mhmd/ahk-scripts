...
^!m::
{
    ...
    if (IsDownloading) {
        result := MsgBox("A download is currently in progress.`n`nURL: " videoURL "`n`nDo you want to add this video to the download queue?",
            "Add to Queue?", 4 + 32)
        if (result != "Yes")
            return
    }

    ; Step 1.6: Continue with URL processing
    ; Step 1.6: Continue with URL processing
    oldClipboard := A_Clipboard
    A_Clipboard := ""
    Send("^l")
