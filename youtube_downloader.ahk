...
; Check for FFmpeg on startup - script won't run without it
CheckFFmpegOnStartup() {
    ffmpegInfo := GetFFmpegLocation()
    if !ffmpegInfo["found"] {
        result := MsgBox(
            "FFmpeg is required to run this script!`n`nFFmpeg was not found in the script directory.`n`nDownloading FFmpeg now...",
            "FFmpeg Required", 64)

        ; Automatically download FFmpeg
        if !DownloadFFmpeg(1, 1) {
            MsgBox("Failed to download FFmpeg. The script will now exit.", "Error", 16)
            ExitApp
        }
        MsgBox("FFmpeg has been successfully installed!`n`nYou can now use the script.", "Setup Complete", 64)
    }
}

; Run FFmpeg check immediately on script start
CheckFFmpegOnStartup()
...
^!m::
{
    ...
    if (!useSameConfig) {
        ...
        if (formatChoice = "mp4") {
            ...
            btn4K := qualityGui.Add("Button", "w120 h35", "4K (2160p)")
            btn1440p := qualityGui.Add("Button", "w120 h35 x+10 yp", "1440p")
            btn1080p := qualityGui.Add("Button", "w120 h35 x+10 yp Default", "1080p")

            btn720p := qualityGui.Add("Button", "w120 h35 xm", "720p")
            btn480p := qualityGui.Add("Button", "w120 h35 x+10 yp", "480p")
            ...
            btn480p.OnEvent("Click", (*) => (videoQuality := "480", qualityGui.Destroy()))
    ...
    if !FileExist("yt-dlp.exe") {
        ...
        if (result = "Yes") {
            if !DownloadYtDlp(1, 1) {
                return
            }
            MsgBox("yt-dlp has been successfully installed!", "Setup Complete", 64)
        } else {
            MsgBox(
    ...
    if (formatChoice = "mp3") {
        ...
        ; Handle quality selection properly
    ...
    if (formatChoice = "mp4") {
        ; Only add quality tag if NOT 1080p (1080p is standard)
        qualityTag := (videoQuality = "1080") ? "" : "_" videoQuality "p"
...
BrowseForFolderNew(parentGui, textControl, &pathVar) {
    ...
    ; Use the modern Windows folder picker COM object for larger, better dialog
    shell := ComObject("Shell.Application")
    folder := shell.BrowseForFolder(0, "Select Download Folder", 0x10 | 0x40 | 0x4000, textControl.Value)
    ...
    ; Fallback to standard dialog if COM fails
    if (selectedFolder = "") {
